...
## Codex Bridge Quick Start

This repository now includes a Codex bridge/runtime layer so you can run PAI workflows in Codex with a high-equivalence compatibility mode.
...
### 2) Generate or refresh AGENTS context (optional but recommended)

```powershell
py Tools/codex_bridge_generator.py --project-root . --output AGENTS.md --force
```

### 3) Launch Codex in PAI runtime mode

```powershell
.\Codex\runtime\pai-codex.ps1
```

### 4) Auto mode for daily usage

If your PowerShell profile includes the `PAI CODEX MODE` block, running `codex` will:

1. Auto-enter PAI runtime mode
2. Auto-inject `--dangerously-bypass-approvals-and-sandbox`

Use `codex-raw` if you need to bypass the wrapper and call native Codex directly.

### 5) Known compatibility boundary

This is an external runtime simulation of Claude lifecycle hooks, not a Codex kernel-native hook implementation. Most hooks work, but some transcript-dependent stop/session-end hooks may log non-blocking warnings.

See:
- [Codex Bridge Overview](Codex/README.md)
- [Codex Runtime Details](Codex/runtime/README.md)
